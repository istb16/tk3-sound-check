 ...
 * 範囲（卓上マイク +10dB）にすると誤差が露出した:
 *
 *   近似開始 -3dB   bias -0.229  MAE 0.260  (n=48)
 *   近似開始 -6dB   bias -0.126  MAE 0.170  (n=46)
 *   近似開始 -8dB   bias -0.067  MAE 0.141  (n=43)
 *   近似開始 -10dB  bias -0.001  MAE 0.096  (n=40)  ← 採用
 *   近似開始 -12dB  bias +0.086  MAE 0.118  (n=32)
 *
 * 深く待つほど測定できる条件が減る（RT60 0.7秒以上で減衰イベントが足りなくなる）。
 * -10dB は誤差が最小になり、単調性も最良(-0.937)になる点。測れなかった条件は
 * 参考値として開示される。
 */
const DECAY_START_DROP_DB = 10;
