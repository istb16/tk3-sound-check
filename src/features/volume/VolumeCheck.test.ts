...
import { startMonitor } from '../../lib/audio.ts';
...
function sine(seconds: number, amplitude: number): Float32Array {
  ...
  for (let i = 0; i < n; i++) out[i] = amplitude * Math.sin((2 * Math.PI * 1000 * i) / SR);
  return out;
}
